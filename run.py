...
def run_day(
    ...
    if example:
        input_file = Path(__file__).parent / "examples" / f"{day:0>2}.txt"
        input_ = input_file.read_text()
    else:
        input_file = Path(__file__).parent / "inputs" / f"{day:0>2}.txt"
        input_ = input_file.read_text()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--example", required=False, action="store_true")
